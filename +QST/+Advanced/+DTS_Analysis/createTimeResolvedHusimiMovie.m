function [] = createTimeResolvedHusimiMovie(X1,X2,Times_1,Times_2,EdgeIndices_1,EdgeIndices_2,TimeStart,TimeEnd,nQuadratures,nResolution,nLimits,Options)
    arguments
        X1;
        X2;
        Times_1;
   ...
   nSetsOfData = floor(length(X_Indices_select)/nQuadratures);
   ...
   for i = 1:nSetsOfData
       % Select Data Subset
       X1_Use = X1(X_Indices_select((i-1)*nQuadratures+1:i*nQuadratures));
       X2_Use = X2(X_Indices_select((i-1)*nQuadratures+1:i*nQuadratures));

       N1_Use = mean(X1_Use.^2)-1;
       N2_Use = mean(X2_Use.^2)-1;
       %Time_Use = Time_1_Select(1)+(i-0.5)*(nQuadratures/(75.3864*10^6));
       Time_Use = (EdgeIndices_1_Select(1)-1+(i-0.5)*nQuadratures)/(75.3864*10^6);
       ...
       [HusimiQ,~,~,~,nTherm,nThermErr,nCoherent,nCoherentErr,meanN,~,~,g2,Coherence,CoherenceErr,~,poissonErrorCut,HusimiCut,HusimiCutTheory] = QST.Plotting.calcHusimiAfterCarolin(X1_Use,X2_Use,[],[],Edges1= Edges1,Edges2=Edges2,CalcStatistics=true);
       DataSets{i} = HusimiQ;
       N1_Sets(i) = N1_Use;
       N2_Sets(i) = N2_Use;
   ...
   for i = 1:nSetsOfData
       ...
       %subplot(212)
       %plot(Time_Sets(1:i),N1_Sets(1:i));
       %xlim([Time_Sets(1),Time_Sets(end)])
       %ylim([0,max(N1_Sets)+1])
       %xlabel('t in s')
       %ylabel('Photonumber')
       %Frame = getframe(gcf);
