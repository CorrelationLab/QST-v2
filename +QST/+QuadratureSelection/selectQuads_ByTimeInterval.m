function [Times_Select,N_Select,G2_Select,EdgeIndices_Select,Indices_X_Select,X_Select,TimeStart,TimeEnd,TimeStartInd,TimeEndInd] = selectQuads_ByTimeInterval(TimeStart,TimeEnd,Times,N,G2,EdgeIndices,X)
    ...
    if ~isempty(EdgeIndices)
        ...
        if ~isempty(X)
            [X_Select,Indices_X_Select] = QST.QuadratureSelection.selectQuads_ByEdgeIndices(X,EdgeIndices_Select);
        else
            [~,Indices_X_Select] = QST.QuadratureSelection.selectQuads_ByEdgeIndices([],EdgeIndices_Select);
            X_Select = [];
        end
    else
        EdgeIndices_Select = [];
